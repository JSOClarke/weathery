...
export async function weatherApi({ longitude, latitude }: Coordinates) {
  // fetch the actual data

  const longitudeParam = "longitude=" + longitude;
  const latitudeParam = "latitude=" + latitude;

  const res = await fetch(
    `https://api.open-meteo.com/v1/forecast?${
      latitudeParam + "&" + longitudeParam
    }&daily=weather_code,temperature_2m_mean&hourly=temperature_2m&current=temperature_2m,rain,weather_code,wind_speed_10m,apparent_temperature,relative_humidity_2m,surface_pressure&timezone=GMT`
  );
